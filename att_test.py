...
def ASR(attack_mode:str, *args, **kwargs):
  ...
  for gt, pred in zip(groundtruths, predictions):
    overlaps = bbox_overlaps(torch.from_numpy(gt['bboxes']), torch.from_numpy(pred['bboxes']))
    ...
    res = np.zeros_like(gt_label)
    att_x, att_y = att_idx
    for x, y in zip(att_x, att_y):
      if gt_label[x] == pred_label[y]:
        res[x] = 1
    num_det = len(np.nonzero(res > 0)[0])
    num_gt = len(res)
    total_num_det += num_det
    total_num_gt += num_gt
    
  print('检测率------', (total_num_det / total_num_gt))
...
def main():
    args = parse_args()
    ASR(args.attack_mode, mode = args.mode, device= args.device)
