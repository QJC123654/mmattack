...
from mmdet.core import (anchor, build_assigner, build_bbox_coder,
...
class YOLOV3Head(BaseDenseHead, BBoxTestMixin):
    ...
    def get_bboxes(self,
        ...
        det_results = []
        logit_out = []
        for (bboxes, scores, objectness, log_) in zip(flatten_bboxes,
                                                flatten_cls_scores,
                                                flatten_objectness, logit):
            ...
            inds = inds // 80
            log_ = log_[inds]
            logit_out.append([log_, cls])
            det_results.append(tuple([det_bboxes, det_labels]))
        return det_results, logit_out
